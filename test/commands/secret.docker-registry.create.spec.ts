...
describe('commands / secret / docker-registry / create', () => {
  ...
  it('should load a secret from the filesystem when --from is defined', async () => {
    ...
    await cmd.handler({ name: 'mynewsecret', from: 'from', ci: true });
    expect(cLoad).toHaveBeenCalledWith(
      posix.join(Filepathes.dockerSecretPath, 'from'),
    );
  });
