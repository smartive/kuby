...
describe('commands / namespace / create', () => {
  ...
  beforeAll(() => {
    ...
    kubeConfigCommand = jest
      .spyOn(namespaceKubeConfigCommand, 'handler')
  ...
  it('should fail if the given name already exists', async () => {
    await namespaceCreateCommand.handler({ name: 'ns1' });
    expect(process.exit).toHaveBeenCalledWith(1);
    expect((Logger as any).instance.error).toHaveBeenLastCalledWith(
      'The namespace "ns1" already exists.',
    );
  });
  ...
  it('should load the default role file if one exists', async () => {
    ...
    await namespaceCreateCommand.handler({ name: 'ns3' });
    expect(
      (prompt as jest.Mock).mock.calls[0][0].map((q: any) => q.default)[2],
    ).toBe('foobar');
  });
  ...
  it('should use biased information when using no-interaction mode', async () => {
    await namespaceCreateCommand.handler({ name: 'ns3', noInteraction: true });
  ...
  it('should write the default role file when user selects to save the role', async () => {
    ...
    await namespaceCreateCommand.handler({ name: 'ns3' });
    expect(vol.readFileSync(Filepathes.namespaceDefaultRolePath, 'utf8')).toBe(
      'myNewRole',
    );
  });
  ...
  it('should fail if no role name is provided', async () => {
    ...
    await namespaceCreateCommand.handler({ name: 'ns3' });
    expect(process.exit).toHaveBeenCalledWith(1);
    expect((Logger as any).instance.error).toHaveBeenCalledWith(
      'No valid role name provided. aborting.',
    );
  });
