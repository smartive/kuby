...
describe('commands / delete', () => {
  ...
  beforeAll(() => {
    ...
    kubeConfig = jest
      .spyOn(kubeConfigCommand, 'handler')
      .mockResolvedValue(undefined);
  });
  ...
  it('should return when used with completion args', async () => {
    expect(
      await deleteCommand.handler({ getYargsCompletions: true } as any),
    ).toBeUndefined();
