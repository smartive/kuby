...
describe('commands / namespace', () => {
  ...
  beforeAll(() => {
    ...
    currentNamespace = jest.spyOn(Kubens, 'getCurrentNamespace').mockResolvedValue('current');
  ...
  it('should do nothing if the selected is the same as current', async () => {
    ...
    expect((Logger as any).instance.info).toHaveBeenLastCalledWith('No different namespace selected, exiting.');
  });

  it('should fail if the namespace does not exist', async () => {
    await namespaceCommand.handler({ name: 'foo' } as any);
    expect((Logger as any).instance.error).toHaveBeenLastCalledWith('The namespace "foo" does not exist.');
    expect(process.exit).toHaveBeenCalledWith(1);
  });

  it('should switch the namespace of the context', async () => {
    await namespaceCommand.handler({ name: 'ns2' } as any);
    expect(exec).toHaveBeenCalledWith('kubectl config set-context "context" --namespace="ns2"');
  });
});
