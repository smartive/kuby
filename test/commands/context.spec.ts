...
describe('commands / context', () => {
  ...
  it('should return when used with completion args', async () => {
    expect(
      await contextCommand.handler({ getYargsCompletions: true } as any),
    ).toBeUndefined();
  ...
  it('should ask the user if no <context> is set', async () => {
    ...
    (exec as jest.Mock)
      ...
      .mockResolvedValueOnce(`current${EOL}non-current${EOL}third`);
  ...
  it('should not ask the user if <context> is set', async () => {
    ...
    await contextCommand.handler({ name: 'current' });
  ...
  it('should not do anything if <context> is current', async () => {
    ...
    expect((Logger as any).instance.info.mock.calls[0][0]).toBe(
      'No different context selected, exiting.',
    );
  });
  ...
  it('should exit if <context> does not exist', async () => {
    ...
    await contextCommand.handler({ name: 'foobar' });
  ...
  it('should switch then context if <context> does exist', async () => {
    ...
    await contextCommand.handler({ name: 'third' });
