...
describe('commands / kubectl / remove', () => {
  ...
  it('should ask the user if no semver version is provided', async () => {
    ...
    await kubectlRemoveCommand.handler({});
    expect(prompt).toHaveBeenCalled();
  });

  it('should warn when the version is not locally installed', async () => {
    await kubectlRemoveCommand.handler({ semver: '2' });
    expect((Logger as any).instance.warn).toHaveBeenLastCalledWith(
      'The given semver is not locally installed.',
    );
  });

  it('should remove the version folder', async () => {
    await kubectlRemoveCommand.handler({ semver: '1.8' });
    expect(Object.keys(vol.toJSON())).not.toContain(
      posix.join(Filepathes.kubectlInstallPath, 'v1.8.4', 'kubectl'),
    );
  });
